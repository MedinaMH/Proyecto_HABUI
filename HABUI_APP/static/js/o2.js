document.addEventListener("DOMContentLoaded", function () {
    ...
    function lineChartO2(containerId) {
        ...
        // Zonas de oxígeno en el fondo (actualizadas con nuevos rangos)
        const o2ZonesData = [
            // Zona crítica (rojo)
            {min: 0, max: 17.0, color: "rgba(239, 68, 68, 0.08)", label: "CRÍTICO"},
            // Zona de advertencia inferior (amarillo)
            {min: 17.0, max: 19.4, color: "rgba(251, 191, 36, 0.08)", label: "ADVERTENCIA"},
            // Zona óptima (verde)
            {min: 19.5, max: 23.5, color: "rgba(16, 185, 129, 0.08)", label: "ÓPTIMO"},
            // Zona de advertencia superior (amarillo)
            {min: 23.6, max: 25.0, color: "rgba(251, 191, 36, 0.08)", label: "ADVERTENCIA"},
            // Zona crítica superior (rojo)
            {min: 25.0, max: 100, color: "rgba(239, 68, 68, 0.08)", label: "CRÍTICO"}
        ];
        ...
        // Función para obtener el color según el nuevo rango
        function getColorForValue(value) {
            if (value >= 19.5 && value <= 23.5) return "#10b981"; // Verde - Óptimo
            if ((value >= 17.0 && value <= 19.4) || (value >= 23.6 && value <= 25.0)) return "#fbbf24"; // Amarillo - Advertencia
            return "#ef4444"; // Rojo - Crítico
        }

        // Función para obtener la etiqueta según el nuevo rango
        function getLabelForValue(value) {
            if (value >= 19.5 && value <= 23.5) return "ÓPTIMO";
            if ((value >= 17.0 && value <= 19.4) || (value >= 23.6 && value <= 25.0)) return "ADVERTENCIA";
            return "CRÍTICO";
        }
        ...
        function redraw() {
            ...
            // Ajustar dominio Y para mostrar mejor los rangos
            const minVal = Math.min(15.0, d3.min(visibleData, d => d.value) - 1.0);
            const maxVal = Math.max(26.0, d3.max(visibleData, d => d.value) + 1.0);
            ...
            points.enter()
                ...
                .on("mouseover", function(event, d) {
                    ...
                    const nivelTexto = getLabelForValue(d.value);
                    const nivelColor = getColorForValue(d.value);
                        ...
                        .html(`
                            ...
                            <div style="color: #94a3b8; margin-bottom: 4px;">
                                ...
                                <span style="margin-left: 8px; font-size: 11px;">
                                    ${nivelTexto === "ÓPTIMO" ? '⭐ ' : nivelTexto === "ADVERTENCIA" ? '⚠️ ' : '🚨 '}
                                </span>
                            </div>
                            ...
                            ${nivelTexto === "ADVERTENCIA" ? '<div style="margin-top: 8px; padding: 4px 8px; background: rgba(251, 191, 36, 0.1); border-radius: 4px; font-size: 10px; color: #fbbf24;">Riesgo fisiológico leve</div>' : 
                              nivelTexto === "CRÍTICO" ? '<div style="margin-top: 8px; padding: 4px 8px; background: rgba(239, 68, 68, 0.1); border-radius: 4px; font-size: 10px; color: #ef4444;">Riesgo severo de hipoxia o incendios</div>' : ''}
                        `)
